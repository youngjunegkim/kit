...
const evidenceDisclosureRules = [
  ...
  },
  {
    ...
    minScore: 2,
    directPatterns: [/과학실\s*분실물함\s*기록|분실물함\s*기록|분실물로\s*접수|분실물.*USB|USB.*분실물|유에스비.*분실물|저장장치.*분실물/],
    patterns: [/과학실/, /분실물함|분실물|USB|유에스비|저장장치|접수/],
    leakPatterns: [/과학실\s*분실물함\s*기록|분실물함\s*기록|분실물로\s*접수|분실물.*USB|USB.*분실물|유에스비.*분실물|저장장치.*분실물/]
  },
  {
    id: "gymPracticeNote",
  ...
  },
  {
    ...
    patterns: [/체육관/, /USB|유에스비|저장장치|최다니엘|CCTV|씨씨티비|작은\s*물건/],
...
function timelineGuideFor(personaId, message) {
  ...
  const exactTimeByPersona = {
    ...
    seoHarin: [
      ...
      "- 오후 6시 10분쯤에는 이상한 자료의 출처를 확인하려 했지만 혼자 판단하기 어려웠다고 답한다. 학생이 먼저 AI 자료 열람 기록을 말했을 때만 자료 열람이라고 표현한다.",
    ...
    choiDaniel: [
      ...
      "- 오후 6시 5분쯤에는 체육관 근처에서 USB로 보이는 작은 저장장치를 주웠고, 오후 6시 10분쯤에는 그 물건을 맡기러 갔다고 말할 수 있다. 학생이 먼저 분실물 기록을 말했을 때만 분실물 접수라고 표현한다.",
  ...
  const noTimeByPersona = {
    ...
    seoHarin: [
      ...
      "- 정확한 시각, AI 자료 열람 기록, 포스터 파일은 학생이 먼저 해당 단어나 증거카드 내용을 말했을 때만 꺼낸다."
    ],
    choiDaniel: [
      "- 최다니엘의 넓은 동선 답변: 과학실에서 보고서를 정리했고 제출할 일이 있어 교무실 근처에 갔다고만 말한다.",
      "- 정확한 시각, 분실물, USB로 보이는 저장장치는 학생이 먼저 해당 단어나 증거카드 내용을 말했을 때만 꺼낸다."
    ]
  };
...
function timelineFallbackReplyFor(message, payload = {}) {
  ...
  if (personaId === "choiDaniel") {
    ...
    if (/6\s*시\s*5\s*분|6\s*시\s*5|여섯\s*시\s*오/.test(raw)) {
      return "6시 5분쯤에는 체육관 근처에서 USB로 보이는 작은 저장장치를 주웠어. 바로 뭔가 수상한 일을 하려던 건 아니었고, 6시 10분쯤 그 물건을 맡기러 갔어.";
    }
...
function personaFallbackReplyFor(message, payload = {}) {
  ...
  if (personaId === "choiDaniel") {
    if (hasFocus("USB/외부 저장장치") || hasFocus("CCTV/USB/작은 물건") || /USB|유에스비|작은\s*물건|분실물|주웠|CCTV|영상/.test(raw)) {
      return "USB처럼 보이는 물건을 주운 건 맞지만, 그게 바로 사건을 저지른 증거라는 뜻은 아니야. 나는 그걸 숨기려던 게 아니라 분실물로 처리하려고 했고, 시험 문제를 만들거나 유출한 건 아니었어.";
    }
