(function () {
  ...
  const standardNote = [
    "범인은 강우진이다.",
    "범인은 학습 도우미 AI를 사용해 교무실에서 본 기말고사 문제지를 태블릿으로 촬영한 뒤, 미술실에서 AI에게 비슷한 유형의 기말 예상 문제로 바꾸게 해서 시험 예상 문제가 유출되었다.",
    "그 근거는 전 여자친구의 메시지, 교무실 책상 위 기말고사 문제지, 오후 5시 45분 태블릿 촬영 장면, 오후 6시 15분 삭제된 AI 프롬프트 기록이다.",
    "범인에게 가장 부족했던 AI 윤리 역량은 신뢰성 또는 주체성이며,",
    "그 이유는 AI를 어떤 목적과 의도로 사용할지 스스로 올바르게 판단하지 못하고, 성적을 위해 시험 문제를 AI에 입력하는 잘못된 선택을 했기 때문이다."
  ].join("\n");
  ...
  const rubric = [
    ...
    },
    {
      ...
      max: 22,
      checks: [
        { label: "학습 도우미 AI 사용", points: 4, patterns: [/학습\s*도우미/, /학교\s*ai/i, /ai/i, /인공지능/] },
        { label: "교무실에서 본 기말고사 문제지를 태블릿으로 촬영", points: 6, patterns: [/교무실.*(기말|시험|문제지).*(태블릿|테블릿|촬영|찍|사진)/, /(태블릿|테블릿).*(촬영|찍|사진).*(기말|시험|문제지)/, /(기말|시험|문제지).*(태블릿|테블릿).*(촬영|찍|사진)/, /문제지.*사진/] },
        { label: "미술실에서 AI에게 변환 요청", points: 6, patterns: [/미술실.*(ai|학습\s*도우미|프롬프트|요청|입력)/i, /(ai|학습\s*도우미|프롬프트|요청|입력).*미술실/i] },
        { label: "비슷한 유형의 기말 예상 문제로 바꿈", points: 6, patterns: [/비슷한\s*유형/, /비슷.*문제/, /유사.*문제/, /기말.*예상/, /예상\s*문제/, /바꾸/, /변환/, /문제.*만들/, /만들어줘/, /생성/] }
      ]
    },
    {
      id: "leak",
    ...
    },
    {
      ...
      max: 26,
      checks: [
        { label: "전 여자친구의 메시지", points: 6, patterns: [/전\s*여자친구.*메시지/, /전여친.*메시지/, /여친.*메시지/, /전\s*여자친구/, /전여친/] },
        { label: "교무실 책상 위 기말고사 문제지", points: 6, patterns: [/교무실.*책상.*(기말|시험|문제지)/, /책상.*(기말|시험|문제지)/, /교무실.*(기말|시험|문제지)/] },
        { label: "오후 5시 45분 태블릿 촬영 장면", points: 6, patterns: [/5\s*시\s*45\s*분.*(태블릿|테블릿|촬영|찍|사진)/, /17:45.*(태블릿|테블릿|촬영|찍|사진)/, /(태블릿|테블릿|촬영|찍|사진).*5\s*시\s*45\s*분/, /(태블릿|테블릿|촬영|찍|사진).*17:45/] },
        { label: "오후 6시 15분 삭제된 AI 프롬프트 기록", points: 8, patterns: [/6\s*시\s*15\s*분.*(삭제|프롬프트|ai|기록)/i, /18:15.*(삭제|프롬프트|ai|기록)/i, /삭제.*(ai\s*)?프롬프트/i, /프롬프트.*기록/, /문제지.*만들어줘/, /비슷한\s*유형.*만들/] }
      ]
    },
    {
      id: "ethics",
      ...
      max: 18,
      checks: [
        { label: "부족한 AI 윤리 역량을 신뢰성 또는 주체성으로 제시", points: 10, patterns: [/신뢰성/, /주체성/] },
        { label: "AI 사용 목적과 의도를 올바르게 판단하지 못한 이유 제시", points: 8, patterns: [/목적.*판단/, /의도.*(판단|사용)/, /올바르게.*판단/, /스스로.*판단/, /성적.*(AI|ai|입력|선택)/, /시험\s*문제.*(AI|ai|입력)/, /잘못된\s*선택/] }
      ]
    }
  ];
  ...
  function coinBonusOf(source) {
    const bonus = source?.questionCreditBonus || null;
    const credits = cleanRemainingCredits(bonus?.credits ?? source?.remainingCredits);
    const score = Number.isFinite(Number(bonus?.score)) ? Number(bonus.score) : credits;
  ...
  function teamLabel(name) {
    ...
    if (!value) return "팀";
    return value.endsWith("팀") ? value : `${value}팀`;
  }

  function setRevealScore(score, source = null) {
    if (!elements.revealScore) return;
  ...
  function fallbackReports(teams) {
    return teams.map((team) => ({
      ...
      questionCreditBonus: team.questionCreditBonus,
  ...
  function animateRevealScore(targetScore, report = null) {
    const target = Math.max(0, Number(targetScore) || 0);
    const reduceMotion = window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches;
    if (reduceMotion) {
      setRevealScore(target, report);
      elements.revealBar?.style.setProperty("--reveal-score", `${scoreBarValue(target)}%`);
    ...
    return new Promise((resolve) => {
      ...
      function frame(now) {
        ...
        setRevealScore(current);
        elements.revealBar?.style.setProperty("--reveal-score", `${scoreBarValue(target * eased)}%`);
        if (progress < 1) {
  ...
  async function showRevealReport(index) {
    ...
    setRevealScore(0);
    elements.revealBar?.style.setProperty("--reveal-score", "0%");
    setRevealText(elements.revealSummary, `${teamLabel(report.name)}의 사건노트를 기준 사건노트와 비교하는 중입니다.`);
  ...
  function animateScoreBars() {
    ...
    if (reduceMotion) {
      cards.forEach((card) => {
        ...
        card.querySelector(".score-bar")?.style.setProperty("--score-width", `${scoreBarValue(target)}%`);
        card.classList.remove("is-measuring");
      });
      return Promise.resolve();
    }
    ...
    return Promise.all(cards.map((card, index) => new Promise((resolve) => {
      ...
      window.setTimeout(() => {
        ...
        function frame(now) {
          ...
          if (scoreBar) scoreBar.style.setProperty("--score-width", `${scoreBarValue(target * eased)}%`);
          if (progress < 1) {
            requestAnimationFrame(frame);
            return;
          }
          if (scoreText) scoreText.textContent = `${formatScore(target)}%`;
          if (scoreBar) scoreBar.style.setProperty("--score-width", `${scoreBarValue(target)}%`);
          card.classList.remove("is-measuring");
          resolve();
        }
        requestAnimationFrame(frame);
